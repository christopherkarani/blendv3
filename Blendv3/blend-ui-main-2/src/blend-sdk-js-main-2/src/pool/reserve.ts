...
export abstract class Reserve {
  ...
  /**
   * Calculate APY from asset data using enhanced calculation logic
   * @param assetData - The asset data containing utilization and rate information
   * @param backstopTakeRate - The backstop take rate as a fixed-point number
   * @param isSupply - Whether calculating for supply (true) or borrow (false) APY
   * @returns The calculated APY as a Decimal with high precision
   */
  public calculateAPYFromAssetData(
    ...
    let totalLiabilities = FixedMath.mulFloor(
      assetData.dSupply,
      assetData.dRate,
      FixedMath.SCALAR_9
    );
    ...
    let utilization = FixedMath.divFloor(
      totalLiabilities,
      totalSupply + totalLiabilities,
      FixedMath.SCALAR_7
    );

    // Calculate current interest rate using the same logic as setRates
    let currentIR = this.calculateCurrentInterestRate(utilization, assetData);
    ...
    if (isSupply) {
      ...
      let supplyRate = FixedMath.mulFloor(
        currentIR,
        supplyCapture,
        FixedMath.SCALAR_7
      );
      apr = FixedMath.toFloat(supplyRate, 7);
    } else {
      // Borrow APR is the current interest rate
      apr = FixedMath.toFloat(currentIR, 7);
    }

    // Apply safety bounds to prevent unrealistic rates
    let maxAPR = 10.0; // 1000% max APR
    let boundedAPR = Math.min(apr, maxAPR);
    ...
    let apy = this.convertAPRtoAPY(boundedAPR, compoundingPeriods);
   ...
   * @param utilization - Current utilization as fixed-point number
   * @param assetData - The reserve data
   * @returns Current interest rate as fixed-point number
   */
  private calculateCurrentInterestRate(
    ...
    if (utilization <= 0) {
      return BigInt(this.config.r_base);
    }

    let irModScalar = FixedMath.toFixed(1, this.irmodDecimals);
    let targetUtil = BigInt(this.config.util);
    let fixed95Percent = BigInt(9_500_000); // 95% in 7 decimal fixed-point
    let fixed5Percent = BigInt(500_000); // 5% in 7 decimal fixed-point

    let currentIR: bigint;

    if (utilization <= targetUtil) {
      // Below target utilization
      let utilScalar = FixedMath.divCeil(
        utilization,
        targetUtil,
        FixedMath.SCALAR_7
      );
      let baseRate = FixedMath.mulCeil(
        utilScalar,
        BigInt(this.config.r_one),
        FixedMath.SCALAR_7
      ) + BigInt(this.config.r_base);

      currentIR = FixedMath.mulCeil(
        baseRate,
      ...
      let baseRate = FixedMath.mulCeil(
        utilScalar,
        BigInt(this.config.r_two),
        FixedMath.SCALAR_7
      ) + BigInt(this.config.r_one) + BigInt(this.config.r_base);
      ...
      let extraRate = FixedMath.mulCeil(
        utilScalar,
        BigInt(this.config.r_three),
        FixedMath.SCALAR_7
      );
      let intersection = FixedMath.mulCeil(
        assetData.interestRateModifier,
        BigInt(this.config.r_two) + BigInt(this.config.r_one) + BigInt(this.config.r_base),
        irModScalar
      );
  ...
  /**
   * Convert APR to APY using precise decimal arithmetic
   ...
   */
  private convertAPRtoAPY(apr: number, periods: number): number {
    if (apr < 0 || periods <= 0) {
      return 0;
    }
    ...
    // APY = (1 + APR/n)^n - 1
    let periodsDecimal = periods;
    let aprPerPeriod = apr / periodsDecimal;
    let onePlusRate = 1 + aprPerPeriod;

    // Use precise power calculation
    let compounded = Math.pow(onePlusRate, periodsDecimal);
    let apy = compounded - 1;

    // Apply reasonable bounds (max 10000% APY)
    let maxAPY = 100.0;
    return Math.min(apy, maxAPY);
  }
}
