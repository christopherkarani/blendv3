...
public class AccountChecker {
    ...
    public init(network: NetworkType = .testnet) {
    ...
    public func checkAccount(publicKey: String) async throws -> AccountStatus {
        print("🔍 DEBUG: Checking account: \(publicKey)")
        logger.info("Checking account: \(publicKey)")
        
        // Try to load account from Horizon
        print("🔍 DEBUG: Loading account from Horizon...")
