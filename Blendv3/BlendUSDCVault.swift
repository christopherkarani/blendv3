...
//                let configService = ConfigurationService(networkType: networkType)
//                let networkSvc = NetworkService(configuration: configService)
//                let cacheSvc = DependencyContainer.shared.cacheService
//                let validationSvc = ValidationService()
//                
//                // Since UserPositionService is @MainActor, we need to handle it properly
//                // in the async context, but we don't need 'await' on the initializer itself
//                let userPositionService = await UserPositionService(
...
//            let userPostion = try! await dataService.fetchUserPosition(userId: signer.publicKey)
//            
//            dump(userPostion)
//
