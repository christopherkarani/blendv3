...
class BlendViewModel: ObservableObject {
    ...
    func checkAccountStatus() async {
        ...
        do {
            ...
            logger.info("Account status: \(status.statusMessage)")
            logger.info("- Exists: \(status.exists)")
            logger.info("- XLM Balance: \(status.xlmBalance)")
            logger.info("- USDC Trustline: \(status.hasUSDCTrustline)")
            logger.info("- USDC Balance: \(status.usdcBalance)")
